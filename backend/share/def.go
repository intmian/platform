...
type (
	...
	BaseSetting struct {
		...
		FeishuWebHook  string `toml:"feishu_webhook"`    // 飞书webhook地址
		...
		D1LogAccountID string `toml:"d1_log_account_id"` // D1日志数据库账号ID
		D1LogApiToken  string `toml:"d1_log_api_token"`  // D1日志数据库API Token
		D1LogDBID      string `toml:"d1_log_db_id"`      // D1日志数据库ID
	}
)
