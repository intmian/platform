...
7. New provider capability code lives under `backend/mian_go_lib/tool/ai/v2`; it does not replace the existing `tool/ai` wrapper.
8. `v2.OpenAIProvider` is for OpenAI-compatible providers only, currently `OpenAI` and `DeepSeek`; model availability is read through the SDK Models API with pagination, while reasoning capability lists are source-type based.
9. `v2.OpenAIProvider.Chat` and `ChatStream` expose plain message chat plus reasoning controls and DeepSeek `thinking`; external tools/functions are intentionally left out of the first v2 chat slice, so `AvailableTools()` returns no callable tools.
10. `tool/ai/v2` also owns the first Agent layer: `AgentRegistry` only owns `AgentID -> AgentMother` and `ProviderID -> IProvider` add/get registries, `global.go` only keeps `AgentRegistry` instances, `CoreAgentSetting` carries a `ProviderResolver`, and `BaseAgentSetting.ProviderID` lets each agent resolve its provider at chat time.
11. `BaseAgent.Chat(ctx, content)` is stateful per agent instance: it prepends `SysPrompt`, reuses successful user/assistant history, tries `Models` in order as a fallback list, writes history only after a successful response, and returns errors instead of local fallback content.
