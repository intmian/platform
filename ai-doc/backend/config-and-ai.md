...
## AI realtime transcription endpoints

1. `POST /misc/ai/transcribe/capability` requires `ai` or `admin` and lets the shared frontend select its transport. It returns `mode: realtime` plus `sampleRate: 8000` when the bound STT queue contains a `DashScopeFunASRRealtime` model; otherwise it returns `mode: file`.
2. `GET /misc/ai/transcribe/realtime` upgrades to a cookie-authenticated WebSocket and requires the same permission. The server owns provider credentials, converts the configured provider host to DashScope's fixed `/api-ws/v1/inference` path, and sends a canonical hyphenated UUID as `task_id`.
3. The browser sends mono PCM16 little-endian binary frames at 8 kHz. Text messages use `finish` or `cancel`; the server emits `ready`, `partial`, `final`, `completed`, or `error` JSON events.
4. `partial` is the current mutable sentence, while `final` confirms a sentence with its `sentenceID`. Frontends must retain confirmed sentences and append the current partial sentence to produce a continuous preview. `completed.text` is the authoritative final aggregate.
5. The realtime server tries configured `DashScopeFunASRRealtime` queue items in order. Upstream WebSocket close codes and reasons are retained in startup errors; a connection that closes without a close frame is reported as closing before `task-started` instead of a bare `EOF`.
