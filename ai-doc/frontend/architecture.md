...
2. `frontend/src/common/WhisperButton.tsx` asks `/misc/ai/transcribe/capability` before each recording. It uses `useRealtimeTranscription` for configured realtime Fun-ASR and otherwise preserves the `useAudioRecorder` plus `transcribeAudio` file path. Recordings with no effective voice end without emitting text. A successful final result is emitted through `onText`; when realtime recognition fails after producing visible text, the same callback receives the best-effort aggregate once before the error is reported, so business-specific insertion can preserve it and restore ordinary editing. When rendered without children, it defaults to an icon-only circular button with tooltip/ARIA labeling owned by the shared component; callers can suppress the visual tooltip with `showTooltip` without changing the button interaction or ARIA label. During recording it expands into a pill that shows elapsed `mm:ss`, renders 16 actual PCM bucket peaks from the latest audio frame on the right for realtime and Web Audio recording paths (without a synthesized repeating pattern), and stops/transcribes when clicked again.
...
5. During realtime recognition, `WhisperButton` concatenates all confirmed sentences with the current mutable partial sentence, so sentence-final events do not make the preview disappear. The aggregate appears in the default `实时转写` popover, while callers can suppress that popover with `showRealtimePreview` and render the same aggregate elsewhere through `onPartialText`. All current product callers suppress the floating popover, render the aggregate inside their owning text editor, and keep that editor read-only until recognition leaves its active states. `onPartialText` resets to an empty string when realtime recognition leaves its active states. Normal completion emits the authoritative aggregate through `onText` once; an error with a non-empty aggregate emits that best-effort text through `onText` once and still reports the error, while cancellation does not commit transient text.
