...
5. 已形成本产品需求和设计草案。
6. 已拆分 `issues/`，按账目管理/对账、数据看板、Excel 导入组织实施任务。
7. 已补充 `technical-design.md`，定义前后端技术方案。
8. 已实现 platform 后端 `POST /misc/money/*` 接口、数据结构、持久化、计算、权限和 Excel 导入预览/确认最小闭环。
9. 已实现前端 `/money` 模块，包括账本列表、配置、批次录入、历史、看板和导入页面。
10. 已补充后端单元测试和 handler 级权限/API 测试。
...
3. 银行、支付宝、微信自动同步未实现。
4. 已确认批次审计修改未实现。
5. Excel 导入第一版为模板识别和汇总导入最小闭环，复杂项目映射校正能力待增强。
