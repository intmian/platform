...
export function GetFile(noClip: boolean, onUpload: (file: File) => void) {
    // 如果剪切板里面有图片，则询问是否上传剪切板图片，否则
    // 弹出上传文件的对话框
    if (!noClip && navigator.clipboard && navigator.clipboard.read) {
                        ...
                        Modal.confirm({
                            ...
                            onOk: () => {
                                item.getType(type).then((blob) => {
                                    onUpload(new File([blob], `clipboard-image.${type.split('/')[1]}`, {type}));
                                });
                            },
                            onCancel: () => {
